            ...
            <div
                ...
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    <div class="d-flex align-items-center gap-2">
                        <label class="text-black mb-0 text-nowrap">Delivery Status:</label>
                        <select id="delivery_type" class="form-control form-select" style="min-width: 220px;">
                            <option value="pending" {{ request('delivery_type', 'pending' )==='pending' ? 'selected'
                                : '' }}>
                                Pending Deliveries
                            </option>
                            <option value="delivered" {{ request('delivery_type')==='delivered' ? 'selected' : '' }}>
                                Delivered Bookings
                            </option>
                        </select>
                    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('delivery_type')?.addEventListener('change', function() {
            ...
            url.searchParams.set('delivery_type', this.value);
            window.location = url;
        });

        // Excel Export (exclude action)
        document.getElementById('exportCsv')?.addEventListener('click', () => {
            ...
            XLSX.writeFile(workbook, 'delivery-report-' + new Date().toISOString().slice(0,10) + '.xlsx');
        });
